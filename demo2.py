...
def 打开回调2(窗口序号):#对于方式一的打开回调的返回值: 大于0 表示成功同时还表示PID 小于等于0失败(中断HD扩展_安装插件2 并把这个返回值返回)
    #打开进程的逻辑...
    #...
    print(f"open_callBack2 winIndex:{窗口序号}")
    return HD系统_启动EXE带参数Ex("D:\XTLBB\Bin64","Game.exe","-fl",0,0)#成功 返回PID  失败 返回小于0的值
    return -1
...
#os.system(f"taskkill /PID {24728} /F")


ret = HD扩展_安装插件2(g_全局窗口序号,g_全局接口类型,g_全局PID,64,0,0,打开回调2,检查回调,30000)
...
HD系统_延迟(10000)# 模拟手动关闭一下进程 崩溃 其他原因关闭了


ret = HD扩展_安装插件2(g_全局窗口序号,g_全局接口类型,g_全局PID,64,0,0,打开回调2,检查回调,30000)
print(f"第二次安装(进程不存在的情况){ret}")#大于0 PID 小于等于0 错误值